...
./check-golden test golden            # all golden tests (uses debug build)
RELEASE=1 ./check-golden test golden  # golden tests with release build
./check-golden test golden/basic/expect_stdout.au.toml  # single golden test file
```
...
Aureum is tested by Aureum — the `golden/` directory contains the golden test suite. CI runs `./check-golden test golden` on Ubuntu, macOS, and Windows.
